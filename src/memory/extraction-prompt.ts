...
如果用户**明确给出了时间线索**，把该事实开始成立的时间填进 validFrom 字段：

- 「我上周三搬到杭州」→ validFrom = "2026-09-16"（用当前日期换算）
- 「我从今年开始每周跑步」→ validFrom 为今年的起点（具体日期）
- 「去年三月换了工作」→ 去年 3 月（具体到日时用 1 号）

规则：
- 换算不出来就**留空**（或 null）。留空表示「自记录起有效」，这是诚实的表达。
- ❌ **不要为了填字段而编造日期** —— 那比留空更糟：
  validFrom 会被展示给用户（「X 起有效」），编错会让用户觉得你记错了他的生活。
- 用户没说时间就不填。绝大多数偏好类信息都不需要它。
