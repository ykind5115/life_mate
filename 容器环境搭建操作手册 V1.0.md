...
在项目根目录 `D:\workspace\LifeMate` 下新建：
...
│   └── postgres\
│       └── init\
│           └── 01-extensions.sql      ← 初始化脚本
├── backups\                            ← 备份输出（加入 .gitignore）
├── docker-compose.yml
├── .env
├── .env.example                        ← 提交到 git
└── .gitignore
...
> ⚠️ **`.env` 必须加入 .gitignore**，它含有数据库密码。这个项目保存的是私密生活数据，密码泄露的代价很高。
...
services:
  postgres:
    ...
    ports:
      ...
      - "127.0.0.1:5432:5432"
    volumes:
      - lifemate-pgdata:/var/lib/postgresql/data
  ...
  embedding:
    ...
    restart: unless-stopped
    # bge-m3 需要 8192 上下文；TEI 通过 --max-client-batch-size 等参数控制
    command: ["--model-id", "BAAI/bge-m3", "--port", "80", "--max-client-batch-size", "32"]
    ports:
      - "127.0.0.1:8080:80"
    volumes:
      # 模型权重缓存，避免每次重启重新下载
      - lifemate-hf-cache:/data
    environment:
      HF_HOME: /data
      TZ: Asia/Shanghai
    deploy:
...
| 独立 `lifemate-hf-cache` 卷 | 模型权重几个 GB，不能每次重启重下 |
| 不用 `version:` 字段 | Compose V2 已废弃该字段，写了只会产生 warning |

## 2.4 `.env.example`（提交到 git）
...
## 2.6 初始化脚本 `docker/postgres/init/01-extensions.sql`

创建文件 `docker\postgres\init\01-extensions.sql`：
