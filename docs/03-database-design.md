...
> PostgreSQL 18 起另有显式的 `uuidv4()`；`gen_random_uuid()` 保留为同义函数，两者等价。

选用 UUID v4 的真实理由：

```text
① 主键会出现在 API 响应、URL、导出文件与日志里，必须不可猜测
② 将来可能合并多个本地库（例如两台机器各自积累的数据），随机主键不会碰撞
③ 与「数据可导出、可迁移」的承诺一致，不需要维护中心化的序列
```

> 🟡 C34：本节初稿列了五条理由，其中两条在本项目中不成立，已删除 ——
> 「不暴露数据数量」（本项目没有外部观察者，别人看不到这张表）、
> 「方便分布式扩展」（V1.0 没有中心库，这是为不存在的场景付费）。

## 7.2 为什么不用 UUID v7（时间有序）

UUID v7（RFC 9562）把 48 位毫秒时间戳放在前面，其余为随机位。它的可见好处只有一个：

| 对比项 | UUID v4（本方案） | UUID v7 |
| ---- | ---- | ---- |
| 存储与传输 | 16 字节 | 16 字节（**完全相同**） |
| 不可猜测 | 122 位随机 | 74 位随机（同样不可猜测） |
| B-tree 插入局部性 | 随机页写 | 顺序写，页分裂更少 |
| 信息暴露 | 无 | **创建时间可从 ID 推出** |

v7 的收益在本项目里不成立：

```text
① 数据量：单用户数年 < 50,000 条记忆（§18.1），总量 10^5 量级。
   主键索引只有一两 MB，随机插入的代价被 shared_buffers 吃掉。
② 没有「按 id 顺序扫描」的查询：时间线按 event_time，会话按 created_at / sequence，
   检索走 embedding + RRF，游标分页挂在 created_at 上 —— v7 唯一的大收益用不上。
③ 没有外部观察者，也就没有「索引更紧凑」带来的竞争压力。
```

而它的代价恰好指向本项目的红线：

```text
❌ v7 把创建时间写进 ID。导出 JSON、URL、日志、截图都会带上它，
   等于给每条记忆附了一个「这条是什么时候记下的」标签。
   对以隐私为先的产品，这是反向的取舍。
```

结论：**不为一个用不到的局部性收益，去牺牲时间隐私。**

## 7.3 什么条件下改用 v7

不要凭感觉改。出现下面任一情况时，**先量再改**：

```text
触发条件（满足其一即可考虑）
  ① 单表行数进入百万级，且出现「按 id 顺序扫描」或「按 id 游标分页」的查询
  ② 真的落地了多租户中心库，写入吞吐成为瓶颈
     （届时 §3.2 的部署形态与 §29 的隐私方案都会一起变）

改法（成本很低，所以现在不换也不会被锁死）
  DEFAULT uuidv7()          -- PostgreSQL 18 起原生内置，不需要任何扩展
  同步范围：11 张表的默认值 + 附录 A + Drizzle schema + 本节

改之前必须先做的两件事
  □ 实测 SELECT uuidv7(); 确实可用（本仓库尚未在容器中验证过这一点）
  □ 明确接受「创建时间可由 ID 推出」这一信息暴露，并同步 §29 的隐私说明
```

虽然 V1.0 是单用户，但提前统一 UUID 的格式成本很低 —— 所以 v4 是默认；
而换成 v7 是一个**有明确触发条件的可选优化**，不是"以后再说"的含糊承诺。
